...
void displayTime(Clock12 userClock12, Clock24 userClock24) {  //Function to display side by side clock
    ...
    cout << "*\t12-Hour Clock\t*\t\t";  //Displays 12-hour clock title
    cout << "*\t24-Hour Clock\t*" << endl;  //Displays 24-hour clock title
...
int main() {
    ...
    while (userSelection != 4) {  //Loop continues until user inputs 4
        displayTime(clock12, clock24);  //Displays 12-hour and 24-hour clocks
        ...
        case 4:  //User selects to exit program
            cout << "You have exited the program. Goodbye." << endl;  //Displays exit message
            break;  //Breaks loop
        default:  //Default in case user enters invalid option
            system("CLS");  //Clears screen
            cout << "Invalid Input. Valid options: 1, 2, 3, 4" << endl;  //Displays error message
        }
    }
}